...
- (void)createSceneContents{
    ...
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(kLedgeSideBufferSpacing, brickBase.position.y + 80)
    ...
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(CGRectGetMaxX(self.frame) - kLedgeSideBufferSpacing - ((howMany - 1) * kLedgeBrickSpacing), brickBase.position.y + 80)
                       withHowManyBlocks:howMany
                           startingIndex:ledgeIndex];
    ...
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(CGRectGetMinX
                                                                        (self.frame)+kLedgeSideBufferSpacing, brickBase.position.y+142)
    ...
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(CGRectGetMidX
                                                                        (self.frame)-((howMany * kLedgeBrickSpacing) / 2), brickBase.position.y+152)
                       withHowManyBlocks:howMany startingIndex:ledgeIndex];
    ledgeIndex = ledgeIndex + howMany;
    ...
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(CGRectGetMaxX
                                                                        (self.frame)-kLedgeSideBufferSpacing-((howMany-1)*kLedgeBrickSpacing),
                                                                        brickBase.position.y+142) withHowManyBlocks:howMany startingIndex:ledgeIndex];
    ledgeIndex = ledgeIndex + howMany;
    ...
    if (CGRectGetMaxX(self.frame) < 500)
        ...
        howMany = 28;
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(CGRectGetMinX
                                                                        (self.frame)+kLedgeSideBufferSpacing, brickBase.position.y+224)
                       withHowManyBlocks:howMany startingIndex:ledgeIndex];
    ledgeIndex = ledgeIndex + howMany;
    ...
    [sceneLedge createNewSetOfLedgeNodes:self startingPoint:CGPointMake(CGRectGetMaxX
                                                                        (self.frame)-kLedgeSideBufferSpacing-((howMany-1)*kLedgeBrickSpacing),
                                                                        brickBase.position.y+224) withHowManyBlocks:howMany startingIndex:ledgeIndex];
    ledgeIndex = ledgeIndex + howMany;
...
-(void)didBeginContact:(SKPhysicsContact *)contact{
    ...
    if (( ((firstBody.categoryBitMask & kWallCategory) != 0) && ((secondBody.categoryBitMask & kCoinCategory) != 0) )) {
        ...
        SKBCoin *theCoin = (SKBCoin *)secondBody.node;
        NSUInteger widthCoin = 8;
        
        if (theCoin.position.x < 100) {
            [theCoin wrapCoin:CGPointMake(self.frame.size.width - widthCoin, theCoin.position.y)];
        }
        else {
            [theCoin wrapCoin:CGPointMake(widthCoin, theCoin.position.y)];
        }
...
-(void)checkForEnemyHits:(NSString *)struckLedgeName{
    ...
    // Ratz
    for (int index=0; index <= _spawnedEnemyCount; index++) {
        NSString *nameChildNode = [NSString stringWithFormat:@"ratz%d", index];
        
        [self enumerateChildNodesWithName:nameChildNode
                               usingBlock:^(SKNode *node, BOOL *stop) {
